...
source("/Users/Leonardo/Documents/covid19model-import/covid19model/utils/geom-stepribbon.r")
#---------------------------------------------------------------------------
make_forecast_plot <- function(){
  ...
  data_interventions <- read.csv("/Users/Leonardo/Documents/covid19model-import/covid19model/data/interventions_latam.csv", 
  ...
  for(i in 1:21){
    ...
    estimated_deaths_cf <- colMeans(estimated.deaths.cf[,1:N,i])
    estimated_deaths_cf_li <- colQuantiles(estimated.deaths.cf[,1:N,i], probs=.025)
    estimated_deaths_cf_ui <- colQuantiles(estimated.deaths.cf[,1:N,i], probs=.975)
    
    estimated_deaths_cf_forecast <- colMeans(estimated.deaths.cf[,1:N2,i])[N:N2]
    estimated_deaths_cf_li_forecast <- colQuantiles(estimated.deaths.cf[,1:N2,i], probs=.025)[N:N2]
    estimated_deaths_cf_ui_forecast <- colQuantiles(estimated.deaths.cf[,1:N2,i], probs=.975)[N:N2]
    ...
    data_country <- data.frame("time" = as_date(as.character(dates[[i]])),
                               ...
                               "estimated_deaths_cf_c" =  cumsum(estimated_deaths_cf),
                               "death_min_c" = cumsum(estimated_deaths_cf_li),
                               "death_max_c"= cumsum(estimated_deaths_cf_ui),
                               "estimated_deaths_cf" = estimated_deaths_cf,
                               "death_min" = estimated_deaths_cf_li,
                               "death_max"= estimated_deaths_cf_ui,
                               "rt" = rt,
                               "rt_min" = rt_li,
                               "rt_max" = rt_ui)
    
    
    write.csv(data_country, file = 
                paste0("/Users/Leonardo/documents/covid19model-import/Latam_covar_long_results/csv_files/","estimations_past_cf_",i, 
                       ".csv"), row.names = TRUE)
    ...
    data_country_forecast <- data.frame("time" = times_forecast,
                                        ...
                                        "estimated_deaths_cf_forecast" = estimated_deaths_cf_forecast,
                                        "death_min_forecast" = estimated_deaths_cf_li_forecast,
                                        "death_max_forecast"= estimated_deaths_cf_ui_forecast)
    
    write.csv(data_country_forecast, file = 
                paste0("/Users/Leonardo/documents/covid19model-import/Latam_covar_long_results/csv_files/","estimation_forecast_cf_",i, 
                       ".csv"), row.names = TRUE)
...
#          `curfew`=stan_data$covariate2[1:stan_data$N[i],i],
...
#`partial quarantine`=stan_data$covariate6[1:stan_data$N[i],i]),
...
make_single_plot <- function(data_country, data_country_forecast, filename, country){
  ...
  data_deaths <- data_country %>%
    select(time, deaths, estimated_deaths_cf) %>%
    gather("key" = key, "value" = value, -time)
  
  data_deaths_forecast <- data_country_forecast %>%
    select(time, estimated_deaths_cf_forecast) %>%
    gather("key" = key, "value" = value, -time)
  ...
  p <- ggplot(data_country) +
    ...
    geom_line(data = data_country, aes(x = time, y = estimated_deaths_cf), 
              col = "deepskyblue4") + 
    geom_line(data = data_country_forecast, 
              aes(x = time, y = estimated_deaths_cf_forecast), 
              col = "black", alpha = 0.5) + 
  ...
  ggsave(file= paste0("/Users/Leonardo/Documents/covid19model-import/covid19model/figures/", country, "_forecast_cf_", filename, ".pdf"), 
         p, width = 10)
}
#-----------------------------------------------------------------------------------------------
make_forecast_plot()
